"use client";
// app/page.tsx — Javari Music · CR AudioViz AI · EIN 39-3646201 · May 2026
import { useState } from "react";
const T=[{"i": "\ud83c\udfa4", "l": "Lyric Writer", "d": "Song lyrics any genre", "h": "/lyrics"}, {"i": "\ud83d\udccb", "l": "Press Release", "d": "Music PR announcements", "h": "/press"}, {"i": "\ud83d\udcdd", "l": "EPK", "d": "Electronic press kit", "h": "/epk"}, {"i": "\ud83d\udcf1", "l": "Social", "d": "Music promotion content", "h": "/social"}];
export default function P() {
  ...
  async function go() { if(!i.trim())return; setL(true);setO("");
    try { const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:i}],stream:false,systemOverride:"You are a professional songwriter and music industry expert."})});
  ...
  return (<div style={{minHeight:"100vh",background:"#040912",color:"#e2e8f0",fontFamily:"system-ui"}}>
    ...
    <section style={{background:"linear-gradient(135deg,#1E3A5F,#040912)",padding:"48px 24px 40px",textAlign:"center"}}>
      <h1 style={{fontSize:"clamp(22px,4vw,42px)",fontWeight:900,color:"#fff",margin:"0 0 10px",lineHeight:1.05}}>AI Tools for<br/><span style={{color:"#FF0800"}}>Musicians and Producers</span></h1>
      <p style={{color:"rgba(255,255,255,0.7)",fontSize:15,margin:0}}>Lyrics, press releases, EPKs, and music business documents.</p>
    </section>
    <section style={{maxWidth:700,margin:"0 auto",padding:"24px 20px 0"}}>
      <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.12)",borderRadius:14,padding:"18px 22px"}}>
        <div style={{display:"flex",gap:8}}><input value={i} onChange={e=>setI(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Write song lyrics: country theme of coming home after years away" style={{flex:1,background:"#172D48",border:"1px solid rgba(0,180,216,0.15)",borderRadius:8,padding:"10px 12px",color:"#e2e8f0",fontSize:13,outline:"none",fontFamily:"system-ui"}}/>
